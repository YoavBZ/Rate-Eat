...
export class UsersPage extends Component {
            ...
            <div>
                ...
                <div className="content-section implementation">
                    ...
                    <Dialog header="Restaurant Details" visible={this.props.visible} width="225px" modal={true}
                        ...
                        {this.props.renderRestaurantDialogContent()}
                    </Dialog>
                </div>
            </div>
        );
    ...
    itemTemplate(restaurant, layout) {
        ...
        if (layout === 'list')
            return this.props.renderListItem(restaurant);
        else if (layout === 'grid')
            return this.props.renderGridItem(restaurant);
    }
}
...
const mapStateToProps = (state) => ({
    ...
    restaurants: state.usersPage.get('restaurants'),
    layout: state.usersPage.get('layout'),
    selectedRestaurant: state.usersPage.get('selectedRestaurant'),
    visible: state.usersPage.get('visible'),
...
const mapDispatchToProps = (dispatch) => {
    return {
        changeLayout: (layout) => {
            dispatch(UsersPageActions.changeLayout(layout));
        },
        onSortChange: (event) => {
        ...
        selectRestaurant: (restaurant, visible) => {
            dispatch(UsersPageActions.selectRestaurant(restaurant, visible));
        }
    }
};
